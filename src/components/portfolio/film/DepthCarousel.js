...
const DepthCarousel = ({
  ...
  const onActiveClickRef = useRef(onActiveClick);
  ...
  onActiveClickRef.current = onActiveClick;
  cfgRef.current = {
    count,
    depth,
    spread,
  ...
  const onCardClick = useCallback(
    index => {
      ...
      if (index === focusRef.current) onActiveClickRef.current?.(data[index], index);
      ...
      <div className="depth-carousel__stage" ref={stageRef}>
        {data.map((item, i) => (
          ...
          >
            ...
            />
            {item.playable && (
              <span className="depth-carousel__play" aria-hidden="true">
