...
const HOME_SCENE_IMAGES = [homeBackground, homeTree, homeHouse, homeCatTail, homeSmoke];
...
export default function Home({innerRef, onEnterStudio}) {
   ...
   useLayoutEffect(() => {
      ...
      const updateScale = () => {
         scaleFrame.style.setProperty('--home-canvas-scale', `${scaleFrame.clientWidth / 1672}`);
      };
   ...
   useEffect(() => {
      let isMounted = true;
      preloadImagesWithTimeout(HOME_SCENE_IMAGES, 4500).then(() => {
   ...
   return (
      <Box ref={innerRef} component={'main'} id={'home'} className={Style.homeScene}>
         ...
         <div ref={scaleFrameRef} className={`${Style.homeScaleFrame} ${isSceneReady ? Style.homeScaleFrameReady : ''}`}>
